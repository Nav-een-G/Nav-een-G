# 👋 Hi, I'm Nav

<p align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=24&duration=3000&pause=1000&color=36BCF7&center=true&vCenter=true&width=600&lines=Computer+Science+Engineering+Student;Software+Developer;AI+%26+Machine+Learning+Enthusiast;Full+Stack+Developer;Always+Learning+Something+New" alt="Typing Animation" />
</p>

<p align="center">
  <a href="https://https://github.com/Nav-een-G">
    <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white" />
  </a>
  <a href="https://www.linkedin.com/in/YOUR_LINKEDIN_USERNAME/">
    <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" />
  </a>
</p>

---

## 🚀 About Me

I'm a Computer Science Engineering student passionate about building
practical software solutions and exploring Artificial Intelligence,
Machine Learning, Web Development, and Cloud Technologies.

- 🎓 Computer Science & Engineering Student
- 💻 Interested in Software Development & AI
- 🧠 Exploring Machine Learning and Data Science
- 🌐 Building Web Applications and Smart Systems
- 🔐 Interested in Cybersecurity and Intelligent Systems
- 🚀 Always learning, experimenting, and building projects

---

## 🛠️ Tech Stack

### 💻 Programming Languages

<p>
  <img src="https://skillicons.dev/icons?i=c,cpp,java,python,js,php" />
</p>

### 🌐 Web Development

<p>
  <img src="https://skillicons.dev/icons?i=html,css,js,react,nodejs,express" />
</p>

### 🤖 AI & Machine Learning

<p>
  <img src="https://skillicons.dev/icons?i=python,tensorflow,opencv" />
</p>

### 🗄️ Databases

<p>
  <img src="https://skillicons.dev/icons?i=mysql,mongodb,sqlite,firebase" />
</p>

### ⚙️ Tools & Platforms

<p>
  <img src="https://skillicons.dev/icons?i=git,github,docker,linux,vscode" />
</p>

---

## 📌 Current Projects

### 🔹 Smart Digital Wallet
A smart financial management system for tracking expenses,
monitoring spending patterns, and analyzing budgets.

### 🔹 Smart Mobile Charging Indicator
A hardware-based system designed to indicate charging completion
and remind users to unplug their mobile charger.

### 🔹 AI & Machine Learning Projects
Working on intelligent systems involving anomaly detection,
classification, prediction, and real-world data analysis.

### 🔹 Web-Based Management Systems
Developing practical web applications using modern frontend,
backend, and database technologies.

---

## 📊 GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=YOUR_USERNAME&show_icons=true&theme=tokyonight&hide_border=true" height="180"/>
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=YOUR_USERNAME&layout=compact&theme=tokyonight&hide_border=true" height="180"/>
</p>

---

## 🔥 GitHub Streak

<p align="center">
  <img src="https://streak-stats.demolab.com?user=YOUR_USERNAME&theme=tokyonight&hide_border=true" />
</p>

---

## 🧰 Tools I Use

| Category | Tools |
|---|---|
| IDE | VS Code |
| Version Control | Git, GitHub |
| Database | MySQL, SQLite, Firebase |
| Development | HTML, CSS, JavaScript, React, Node.js |
| Programming | C, C++, Java, Python, PHP |
| AI/ML | Python, TensorFlow, OpenCV |
| Deployment | Docker |
| OS | Windows, Linux |

---

## 📈 Contribution Graph

<p align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=YOUR_USERNAME&theme=tokyo-night&hide_border=true" />
</p>

---

## 🏆 GitHub Trophies

<p align="center">
  <img src="https://github-profile-trophy.vercel.app/?username=YOUR_USERNAME&theme=tokyonight&no-frame=true&no-bg=true&margin-w=5" />
</p>

---

## 🌱 Currently Learning

- Artificial Intelligence
- Machine Learning
- Full Stack Development
- Cloud Computing
- Cybersecurity
- Data Structures & Algorithms
- Software Engineering

---

## 🎯 2026 Goals

- 🚀 Build more real-world projects
- 🧠 Improve AI/ML skills
- 💻 Strengthen Data Structures & Algorithms
- ☁️ Learn more about Cloud Computing
- 🔐 Explore Cybersecurity
- 🤝 Contribute to Open Source
- 📚 Continuously improve my development skills

---

## 📫 Connect With Me

<p align="center">
  <a href="https://github.com/YOUR_USERNAME">
    <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white" />
  </a>
  <a href="https://www.linkedin.com/in/YOUR_LINKEDIN_USERNAME/">
    <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" />
  </a>
</p>

---

## 👀 Profile Visitors

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=YOUR_USERNAME&label=Profile%20Views&color=0e75b6&style=for-the-badge" />
</p>

---

<p align="center">
  <b>⭐ Thanks for visiting my profile!</b>
</p>

<p align="center">
  💻 Build • Learn • Experiment • Repeat 🚀
</p>
